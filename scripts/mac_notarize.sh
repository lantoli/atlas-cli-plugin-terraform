...
if [[ -f "./dist/macos_darwin_amd64_v1/binary" && -f "./dist/macos_darwin_arm64/binary" && ! -f "./dist/binary_signed.zip" ]]; then
	...
	zip -r ./dist/mac_bin.zip ./dist/macos_darwin_amd64_v1/binary ./dist/macos_darwin_arm64/binary # The Notarization Service takes an archive as input
	...
	unzip -oj ./dist/mac_bin_signed.zip dist/macos_darwin_arm64/binary -d ./dist/macos_darwin_arm64/
fi
